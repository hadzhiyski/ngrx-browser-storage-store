...
import { createAction, createReducer, on, UPDATE, INIT } from '@ngrx/store';
...
describe('BrowserStorageMetaReducerLoader', () => {
  [
    ...
    it(`should load from storage on ngrx/UPDATE once Storage: ${test.cfg.storage} Feature: ${test.cfg.feature || 'undefined'}`, () => {
      ...
      expect(storageService.initialStateLoaded).toBeTrue();
    });
  });
});
